 ...
 * Em produção o padrão é **desligado** — o app usa poll em `/api/sync/status`
 * (Render free dorme e o WS gera ruído no console). Ative com NEXT_PUBLIC_ENABLE_SYNC_WS=1.
 */
export function getWsUrl(): string {
  ...
  if (process.env.NEXT_PUBLIC_ENABLE_SYNC_WS !== '1') {
    return '';
  }
