...
import { resolveCarouselOpenIndex } from './carousel-utils';
...
describe('resolveCarouselOpenIndex', () => {
  ...
  it('returns first item with release date >= today when future releases exist', () => {
    const items = [
      mockMidia(1, '2026-07-01'),
      mockMidia(2, '2026-07-15'),
      mockMidia(3, '2026-08-20'),
      mockMidia(4, '2026-09-01'),
    ];
    assert.equal(resolveCarouselOpenIndex(items), 2);
  });

  it('when all releases are past, returns last item of current month', () => {
  ...
  it('when all releases are past and none in current month, returns last item', () => {
    const today = new Date();
    const year = today.getFullYear();
